...
function MenuManager({ menu, catNames, dbOps, showToast, isMobile }) {const{T:C,TF:F}=useAdminTheme();
  ...
  async function handleDrop(e,dropIdx){e.preventDefault();if(dragIdx===null||dragIdx===dropIdx){setDragIdx(null);setOverIdx(null);return;}const dragItem=onMenuSorted[dragIdx];if(!dragItem)return;const reordered=[...onMenuSorted];const [moved]=reordered.splice(dragIdx,1);reordered.splice(dropIdx,0,moved);const ids=reordered.map(i=>i.id);setSaving(prev=>{const n={...prev};ids.forEach(id=>{n[id]=true;});return n;});try{for(let i=0;i<reordered.length;i++){if(reordered[i].menuOrder!==i)await dbOps.updateMenuItem(reordered[i].id,{menuOrder:i});}}catch(e2){console.error(e2);showToast("Reorder failed","error");}finally{setSaving(prev=>{const n={...prev};ids.forEach(id=>{delete n[id];});return n;});}setDragIdx(null);setOverIdx(null);}
