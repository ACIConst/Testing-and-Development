...
function OrderHistory({ orders, users, dbOps, showToast }) {
  ...
  return(
    <div style={{animation:"fadeUp .3s ease"}}>
      ...
      {confirmClear&&<ConfirmModal message={"Archive all "+active.length+" active order"+(active.length!==1?"s":"")+"?"} confirmLabel="Archive All" onConfirm={handleClearAll} onClose={()=>setConfirmClear(false)}/>}       {confirmDelete&&<ConfirmModal message={"Are you sure you want to delete this order? You cannot restore it once deleted."} confirmLabel="Delete Order" danger onConfirm={()=>handleDeleteOrder(confirmDelete)} onClose={()=>setConfirmDelete(null)}/>}
    </div>
  );
}
